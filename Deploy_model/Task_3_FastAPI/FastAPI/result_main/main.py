...
# Địa chỉ Triton Server (HTTP endpoint)
TRITON_HTTP_URL = "172.17.0.2:8000"  # Cổng 8000 cho HTTP
...
async def infer_image(file: UploadFile = File(...)):
    try:
        ...
        infer_input.set_data_from_numpy(np_img, binary_data=True)

        # Yêu cầu output
        infer_output = InferRequestedOutput(OUTPUT_NAME, binary_data=True)

        # Gửi request đến Triton qua HTTP
